...
class Game: ObservableObject {
	...
	var realTurn: Int { gameState == .active || gameState == .new ? moves.count % 2 : (gameState.myWin ? myTurn : (gameState.opWin ? myTurn^1 : 2)) }
	var myTurn: Int = 0
	...
	func load(mode: GameMode, boardNum: Int = 0, turn: Int? = nil, hints: Bool = false, time: Double? = nil, manualPreset: Bool = false) {
		Game.main = self
		gameState = .new
		self.mode = mode
...
//		}
		...
		if manualPreset {
			while true {
				print("load board? (y/n):")
				if readLine() != "y" { break }
				User(b: board, n: board.getTurn()).move()
			}
		}
...
//		let newTurn = newMode == .online ? FB.main.myGameData?.myTurn : mostRecentGame.2
		...
		load(mode: newMode, boardNum: mostRecentGame.1, turn: mostRecentGame.2, hints: mostRecentGame.3, time: mostRecentGame.4)
	}
...
//		}
		let move = Move(p)
		if processingMove { return }
		guard gameState == .active || gameState == .new else { return }
		guard turn == realTurn else { print("Invalid turn!"); return }
		guard setup == moves.map({ $0.p }) else { print("Invalid setup!"); return }
		guard !moves.contains(move) && (0..<64).contains(move.p) else { print("Invalid move!"); return }
